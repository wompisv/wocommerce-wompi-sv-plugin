...
class wompi_Payment_Gateway extends WC_Payment_Gateway {
  ...
  global $woocommerce;
    ...
    add_action( 'admin_notices', array( $this,  'do_ssl_check' ) );
     add_action( 'woocommerce_api_wc_gateway_wompi', array( $this, 'validate_wompi_return' ) );
     add_action( 'woocommerce_api_wc_webhook_wompi', array( $this, 'validate_wompi_webhook' ) );
 ...
 public function validar_hash_wompi($cadena,$order_id,$hash){
       ...
       if ($sig == $hash) {
           ...
           $customer_order->add_order_note( __( 'wompi pago completado.', 'wompi-payment' ) );
            ...
            update_post_meta($order_id , '_wc_order_wompi_transactionid', $idEnlace,true);
            update_post_meta($order_id , '_wc_order_wompi_authcode', $idTransaccion,true );
...
$hash = $headers['Wompi_Hash'];
       ...
       if ($sig == $hash) {
           ...
           $customer_order->add_order_note( __( 'wompi pago completado WH.', 'wompi-payment' ) );
            ...
            $customer_order->payment_complete();
            update_post_meta($order_id , '_wc_order_wompi_transactionid',$arrayResult->{'EnlacePago'}->{'Id'},true);
            update_post_meta($order_id , '_wc_order_wompi_authcode', $arrayResult->{'CodigoAutorizacion'},true );
            $woocommerce->cart->empty_cart();
  ...
  public function validate_wompi_return(){
                global $woocommerce;
                $order_id = $_GET['identificadorEnlaceComercio'];
                  $customer_order = new WC_Order( $order_id);
...
function show_WOMPI_info( $order ){
    ...
    echo '<p><strong>'.__('WOMPI Código de autorización').':</strong> ' . get_post_meta( $order_id, '_wc_order_wompi_authcode', true ) . '</p>';
